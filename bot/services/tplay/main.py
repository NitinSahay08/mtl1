...
class TPLAY:
    ...
    def start_process(self):
        msg = self.message.reply_text(LOG_MESSAGE.format("Processing...", "Channel ", self.parsed_args.channel), parse_mode="Markdown")
    ...
    def check_before_continue(self, msg):
        try:
            ...
            msg.edit_text(LOG_MESSAGE.format("ERROR", "CHANNEL", e), parse_mode="Markdown")
            return False
        ...
        if self.parsed_args.start:
            ...
            if not can_continue:
                msg.edit_text(LOG_MESSAGE.format("ERROR", "CATCHUP", msg_text), parse_mode="Markdown")
                return False
    ...
    def download_catchup(self, msg):
        ...
        msg.edit_text(LOG_MESSAGE.format("Downloading...", "Catchup", name), parse_mode="Markdown")
        ...
        processor = Processor(
            ...
            headers=self.channel_data.get('manifest_headers')
        )
        processor.start_process()
        msg.edit_text(LOG_MESSAGE.format("Download started...", "Catchup", name))
